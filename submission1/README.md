...
| Operational schema modeled: related tables + keys | ✅ | `migrations/001_operational_schema.sql` | **`notebooks/operational_schema_evidence.ipynb`** (executed, outputs: 4 tables, PKs, 5 FKs, row counts) + `results/operational_schema_result.json` |
| Separate writable Postgres tables ≠ read-only synced | ✅ | migration 001 (`northpeak_ops.*`) vs `public.store_sku_position_synced` | **`notebooks/writable_tables_evidence.ipynb`** (executed INSERT+UPDATE, audit trigger, distinct-object proof) + `results/writable_proof.json` |
...
| Main stays clean until promotion (git history) | ✅ | work isolated on `feat/lakebase-rubric` | **`git_evidence/commit_history.txt`** (committed `git log --graph`: feature commits on side branch, main advanced only by merge 4849ccb) |
...
| Rubric row | Status | Code | Evidence |
|---|---|---|---|
| Agent change committed as diff / migration | ✅ | `migrations/001_*.sql`, `migrations/002_*.sql` | **`git_evidence/agent_change.diff`** (committed unified diff of the migrations) |
| Change validated by committed test + result | ✅ | `tests/test_operational_schema.sql`, `09_run_tests.py` | **`notebooks/agent_change_test_validation.ipynb`** (executed, all tests PASS) + `results/test_result.json` |
| Validated change promoted via merge / PR into main | ✅ | PR #1 `feat/lakebase-rubric` → `main` | **`git_evidence/promotion_pr.md`** (committed PR record: state MERGED, merge commit 4849ccb) |
...
python notebooks/_build_notebooks.py  # executed evidence notebooks (outputs committed)
